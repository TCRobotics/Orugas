...
class numericUpDown{
  ...
  */
  void update(){
    ...
    for(int i=0; i < incomingValues.length; i++){ 
      drawValue(i, mouseOverUp(i), mouseOverDown(i), false); 
    }
    ...
    if(!valueSet){
      ...
      for(int i=0; i<incomingValues.length; i++){
        if(incomingValues[i] != values[i]){
          ...
          drawValue(i, mouseOverUp(i), mouseOverDown(i), true);
        }    
      }
    }
  }
  ...
  */
  void drawValue(int id, boolean mouse_over_up, boolean mouse_over_down, boolean onlyValue)
  { 
    int spacing = (20 + valueBoxWidth) * id;
